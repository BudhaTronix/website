...
export default function Hero({ theme, setMode, setIsEasterEgg, isEasterEgg }) {
  ...
  const snapshot = knowledge.profile_snapshot;
  const snapshotChips = [
    { icon: "📍", text: snapshot.location },
    { icon: "🇩🇪", text: snapshot.relocation },
    { icon: "✓", text: "PR holder" },
    { icon: "🗣", text: "EN · BN · HI · DE (B1 in prep)" },
  ];
  ...
  return (
    ...
    >
      ...
      <motion.h1
        ...
        className={`font-display text-3xl sm:text-4xl md:text-6xl lg:text-6xl font-bold leading-tight ${isReverseMorphing ? 'relative z-50 pointer-events-none' : ''}`}
        style={{ color: "var(--text-primary)" }}
      >
        {isReverseMorphing ? "Hello, I'm" : "Hi, I'm"} <span className="gradient-text">Budhaditya{isReverseMorphing ? "" : " Mukhopadhyay"}</span>
      </motion.h1>
      ...
      <motion.img
        ...
        className={`w-40 h-40 sm:w-52 sm:h-52 md:w-64 md:h-64 lg:w-72 lg:h-72 rounded-full object-cover mt-6 md:mt-8 mb-6 md:mb-8 border-4 shadow-lg cursor-pointer hover:shadow-[0_0_30px_var(--accent-glow)] transition-shadow ${isReverseMorphing ? 'relative z-50 pointer-events-none' : ''}`}
      ...
      <motion.p
        ...
        className="text-base md:text-xl lg:text-2xl mb-5"
        style={{ color: "var(--text-secondary)" }}
      >
        <span className="font-semibold" style={{ color: "var(--text-primary)" }}>Senior AI Engineer</span> · <span className="gradient-text font-semibold">GenAI &amp; Edge AI</span> · Strategic Advisor — UAV &amp; AI Innovation
      </motion.p>

      {/* Recruiter snapshot chips + profile links */}
      <motion.div
        initial={{ opacity: isEasterEgg ? 1 : 0 }}
        animate={{ opacity: isReverseMorphing ? 0 : 1 }}
        transition={{ duration: isReverseMorphing ? 0.4 : (isEasterEgg ? 0 : 0.8), delay: isEasterEgg ? 0 : 0.85 }}
        className="flex flex-wrap justify-center items-center gap-2 mb-6 max-w-3xl"
      >
        {snapshotChips.map((chip, i) => (
          <span
            key={i}
            className="px-3 py-1.5 rounded-full text-xs md:text-sm border backdrop-blur-md"
        ...
        <span className="flex items-center gap-2 ml-1">
          <a href={knowledge.contact.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn"
            className="w-8 h-8 rounded-full border flex items-center justify-center transition hover:scale-110 hover:border-[var(--accent)]"
            style={{ backgroundColor: "var(--glass-bg)", borderColor: "var(--glass-border)", color: "var(--text-secondary)" }}>
      ...
      <motion.div
        initial={{ opacity: isEasterEgg ? 1 : 0 }}
        animate={{ opacity: isReverseMorphing ? 0 : 1 }}
        transition={{ duration: isReverseMorphing ? 0.4 : (isEasterEgg ? 0 : 0.8), delay: isEasterEgg ? 0 : 1 }}
        className="flex flex-wrap justify-center gap-3 mb-10"
      >
        <button
          onClick={startReverseEasterEgg}
          className="px-6 py-3 rounded-xl text-white font-semibold shadow-lg transition hover:scale-105 hover:shadow-[0_0_25px_var(--accent-glow)]"
        ...
        <a
          ...
          className="px-6 py-3 rounded-xl font-semibold border transition hover:scale-105 hover:border-[var(--accent)]"
      ...
      {/* Metric stat strip */}
      <motion.div
        className="grid grid-cols-2 sm:grid-cols-4 gap-x-8 gap-y-6 mb-12 md:mb-14 max-w-4xl"
      ...
      >
        {knowledge.metrics.map((m, i) => (
          ...
          >
            <span className="font-display text-2xl md:text-3xl font-bold gradient-text">{m.value}</span>
            <span className="text-[11px] md:text-xs mt-1" style={{ color: "var(--text-secondary)" }}>{m.label}</span>
          </motion.div>
        ))}
      </motion.div>
